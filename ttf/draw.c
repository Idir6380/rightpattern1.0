...
{   int o=0;
    ...
    // Create a SDL window
    SDL_Window *windowmenu = SDL_CreateWindow("DRAW", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1200,900, SDL_WINDOW_MOUSE_FOCUS);

    // Create a renderer (accelerated and in sync with the display refresh rate)
    SDL_Renderer *renderer = SDL_CreateRenderer(windowmenu, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
    SDL_Surface *menu=SDL_LoadBMP("CPULost-1.bmp");
